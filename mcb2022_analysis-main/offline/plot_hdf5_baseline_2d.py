...
with h5py.File(args.filename, "r") as h5file:
    ...
    for x_group in sdr_data_by_x.values():
        ...
        for y_group in x_group.values():
            ...
            seft_flattop_dfs_ppm[row_idx, col_idx] = (seft_flattop_mean - baseline_freq) / f0 * 1e6
            seft_flattop_df_stds_ppm[row_idx, col_idx] = (
                np.sqrt(baseline_freq_std**2 + seft_flattop_std**2) / f0 * 1e6
            )
            seft_exp_n0s_ppm[row_idx] = seft_exp_n0 / f0 * 1e6
            seft_exp_lambdas[col_idx] = seft_exp_lambda
    ...
    cbar.set_label("Baseline frequency (Hz)")
    plt.show()

    #plt.title(f"Mean baseline standard deviation.\nRun {run_id}.")
    #pos = plt.imshow(
    #    baseline_stds,
    #    origin="lower",
    #    extent=im_extent,
    ...
    #plt.xlabel("X position (µm)")
    #plt.ylabel("Y position (µm)")
    #cbar.set_label("Baseline standard deviation (Hz)")
    #plt.show()
    ...
    #)
    #cbar = plt.colorbar(pos)
    #plt.title(f"Number of transients after outlier rejection.\nRun {run_id}.")
    #plt.xlabel("X position (µm)")
    #plt.ylabel("Y position (µm)")
    #cbar.set_label("Number of transients")
    #plt.show()
